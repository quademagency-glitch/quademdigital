...
The preference is persisted to `localStorage` (`main.js:602`), so one tap on the toggle —
which sits beside "Book a Call" on every page — leaves the visitor with an unreadable site
