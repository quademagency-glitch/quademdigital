 ...
 *
 * **There is a second copy of this rule in vercel.json, and both are needed.**
 * On Vercel the routing table is generated by the adapter from the same
 * `trailingSlash` setting, and its last entry is a catch-all that renders the
 * function *and forces a 404 status*. So a rewrite in here produced the right
 * bytes, the right content type and the right headers under a 404, which no
 * amount of correctness inside the function can undo: the status is decided
 * before it runs. The `rewrites` entry in vercel.json moves the request onto
 * the pitch route proper, and this rule is what makes `astro dev` behave the
 * same, since nothing reads vercel.json locally.
 */
