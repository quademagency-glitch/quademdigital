...
export async function settleInvoice(reference: string): Promise<SettleResult> {
  ...
  // 7. Idempotency. The webhook and the browser callback both land here.
  if (String(invoice.status).toLowerCase() === 'paid') {
    if (invoice.paystackReference && invoice.paystackReference !== reference) {
      // Two distinct successful payments for one invoice. Refunding is a human
      // decision, so record loudly and still answer 200 so Paystack stops retrying.
      console.error('[paystack] DOUBLE PAYMENT', {
        invoiceId,
  ...
  if (paidMinor < expectedMinor) {
    console.error('[paystack] underpayment', { invoiceId, paidMinor, expectedMinor })
    return err(409, 'AMOUNT_MISMATCH', 'Payment amount does not match the invoice total.')
  }
  ...
  return ok({ invoiceId, reference, overpaid: paidMinor > expectedMinor })
}
